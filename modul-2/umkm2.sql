...
CREATE TABLE skala_umkm (
    id_skala INT PRIMARY KEY AUTO_INCREMENT,
    nama_skala VARCHAR(100),
...
INSERT INTO kabupaten_kota (nama_kota) VALUES
('Kota Bandung'),
('Kota Bekasi'),
('Kota Bogor'),
('Kota Cimahi'),
...
-- Data untuk UMKM
INSERT INTO umkm (nama_usaha, id_pemilik, id_kategori, id_skala, id_kabupaten, alamat_usaha, nib, npwp, tahun_berdiri, jumlah_karyawan, total_aset, omzet_per_tahun, deskripsi_usaha, tanggal_registrasi) VALUES
...
DELETE FROM skala_umkm;
DELETE FROM kategori_umkm;

DROP TABLE kabupaten_kota;
DROP TABLE kategori_umkm;
DROP TABLE pemilik_umkm;
DROP TABLE produk_umkm;
DROP TABLE skala_umkm;
DROP TABLE umkm;
...
SELECT 
    u.nama_usaha,
    p.nama_lengkap AS pemilik_umkm,
    z.nama_kategori AS kategori_umkm,
    s.nama_skala AS skala_umkm,
    k.nama_kota AS kabupaten_kota,
    u.tahun_berdiri
FROM umkm u
JOIN pemilik_umkm p ON u.id_pemilik = p.id_pemilik
JOIN kategori_umkm z ON u.id_kategori = z.id_kategori
JOIN skala_umkm s ON u.id_skala = s.id_skala
JOIN kabupaten_kota k ON u.id_kabupaten = k.id_kabupaten;

SELECT nama_usaha, pemilik_umkm FROM view_umkm_detail;
...
SELECT
    ...
    e.nama_usaha
FROM pemilik_umkm i
JOIN umkm e ON i.id_pemilik = e.id_pemilik;
